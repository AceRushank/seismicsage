...
export function FilterPills({ filter, onChange }: FilterPillsProps) {
  ...
  const isActive = (pill: PillDef): boolean => {
    if (pill.group === 'magnitude') return filter.magnitude === pill.key
    return filter.time === pill.key
  }

  const handleClick = (pill: PillDef) => {
    if (pill.group === 'magnitude') {
      onChange({ ...filter, magnitude: pill.key as FilterState['magnitude'] })
    } else {
      onChange({ ...filter, time: pill.key as FilterState['time'] })
    }
  }
...
function Pill({ pill, active, layoutId, onClick }: PillProps) {
  return (
    ...
    >
      {/* Sliding active background — Framer Motion layoutId creates the shared-element effect */}
      {active && (
        <motion.span
          layoutId={layoutId}
          style={{
